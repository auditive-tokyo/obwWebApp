...
async def openai_vector_search_with_file_search_tool( # 関数名をより具体的に
    ...
    system_instructions = f"""あなたはOsaka Bay Wheelというホテルの親切なアシスタントです。
ユーザーからの質問に対して、提供された File Search ツールを使って関連情報を検索し、その情報に基づいて {language} で自然な応答を生成してください。
検索結果が見つからない場合や、情報が不十分な場合は、その旨を正直に伝えてください。
"""
    try:
        response_format_schema = {
            ...
            "required": ["assistant_response_text"],
        ...
        response = await openai_async_client.responses.create(
            ...
            tool_choice="required", # File Search を必須にする
            temperature=0.5,
            text={
                "format": {
                    "type": "json_schema",
