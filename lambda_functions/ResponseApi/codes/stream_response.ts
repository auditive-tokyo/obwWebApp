          ...
          schema: {
            ...
            properties: {
              ...
              needs_human_operator: {
                type: "boolean",
                description:
                  "Set to true ONLY when the user answers 'yes' to the question 'Would you like to transfer your inquiry to an operator?' (オペレーターにお問い合わせを転送しますか？). NEVER set to true before asking this confirmation question, even for urgent issues. This triggers a Telegram notification to the operator.",
              },
              inquiry_summary_for_operator: {
                type: "string",
                description:
                  "A concise summary of the guest's inquiry for the human operator, formatted for Telegram messaging. Include key details like the issue type, current situation, urgency level, and any actions needed. If guest information is NOT available in the system prompt, include the guest's contact information (phone or email) obtained during the conversation. If guest information IS available in the system prompt, DO NOT include it here as it will be retrieved from the database. Return empty string \"\" if needs_human_operator is false.",
              },
            },
            required: [
              "assistant_response_text",
