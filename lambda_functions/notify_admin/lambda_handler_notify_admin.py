...
def send_telegram(text: str) -> None:
    ...
    with urlopen(req, timeout=10) as resp:
        ...
        if resp.status != 200:
            raise RuntimeError(f"Telegram send failed: {resp.status} {body_text!r}")

def handler(event, context):
    # DynamoDB Streams レコード配列を処理
    for rec in event.get("Records", []):
        try:
            # DynamoDB Stream のフィルタをテンプレート側で MODIFY のみに絞っているため OldImage/NewImage の遷移のみ。
            ...
            if not (old_status is not None and old_status != "pending" and new_status == "pending"):
            ...
            booking_id = _s(new_img.get("bookingId"))

            # sessionの代表者のみ sessionTokenHash を持つ
            session_token_hash = _s(new_img.get("sessionTokenHash"))
            is_representative = True if session_token_hash else False

            if not is_representative:
                logger.info(f"Skipping non-representative record for guest {guest_id}")
                continue

            lines = [
                f"Room ({room_number}) の {guest_name} さんが基本情報の登録と、IDの写真をアップロードしました。",
                "Admin Pageより確認してください:",
                f"滞在日: {check_in or '-'} ~ {check_out or '-'}",
                "",
                f"{ADMIN_BASE_URL}/{room_number}/{booking_id}",
            ]
            message_text = "\n".join(lines)
