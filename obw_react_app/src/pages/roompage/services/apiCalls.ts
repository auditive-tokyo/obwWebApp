...
export async function refreshGuestSessions({
  client,
  roomId,
  setGuestSessions,
}: Params) {
  if (!roomId) {
    dbg("refreshGuestSessions: no roomId");
    return;
  }
  if (!setGuestSessions) {
    dbg("refreshGuestSessions: no setGuestSessions");
    return;
  }
...
export async function loadMyGuest({
  client,
  roomId,
}: {
  client: Client;
  ...
  try {
    ...
    const res = await client.graphql({
      query,
      variables: { roomNumber: roomId, guestId: gid },
      authMode: "iam",
    } as GraphParams);
    const resObj = res as unknown as GraphqlResponse<{ getGuest?: unknown }>;
    const g = resObj.data?.getGuest as GuestDetail | null;
    dbg(
      "loadMyGuest result ->",
      g ? { name: g.guestName, status: g.approvalStatus } : null,
    );
    return g || null;
  } catch (e) {
...
export async function updateRoomCheckDates({
  ...
  try {
    ...
    const guestsResObj = guestsResult as unknown as GraphqlResponse<{
      listGuestsByBooking?: unknown;
    }>;
    const guestsRaw = guestsResObj.data?.listGuestsByBooking ?? [];
    const guests = Array.isArray(guestsRaw)
      ? guestsRaw.map((g) => g as unknown as GuestSession)
      : [];
