...
type Props = {
  readonly detail: Guest;
  readonly onClose: () => void;
  readonly signing: boolean;
  readonly signedPassportUrl: string | null;
  readonly approvingId: string | null;
  readonly rejectingId: string | null;
  readonly confirmApprove: (g: Guest) => void;
  readonly confirmReject: (g: Guest) => void;
  readonly onUpdate: (updatedGuest: Guest) => Promise<void>;
};
...
export function DetailsModal({
  ...
  const passportUrlContent = signedPassportUrl ? (
    <a href={signedPassportUrl} target="_blank" rel="noreferrer">
      署名付きURLで開く
    </a>
  ) : (
    "画像URLの取得に失敗しました"
              ...
              {detail.passportImageUrl && (
                <>
                  <div style={{ marginBottom: 4 }}>
                    {signing ? "画像URL取得中…" : passportUrlContent}
                  </div>
                  {signedPassportUrl && (
