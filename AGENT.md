...
│   │       ├── system_instructions.ts            # システムプロンプト生成
│   │       ├── intent_classifier.ts              # Intent 分類器 (gpt-5.4-nano)
│   │       ├── operator_transfer_classifier.ts   # オペレーター転送判定 (gpt-5.4-nano)
│   │       └── operational_hours.ts              # 対応時間取得
...
│   ├── AccessMap.md                 # 道案内・交通・USJアクセス（TRANSPORT store）
│   ├── FacilityGuide.md             # 施設案内・設備・チェックイン（FACILITY store）
│   ├── FrequentlyAskedQuestions.md  # FAQ（FACILITY store）
│   ├── GuestServices.md             # 郵便物受取・追加クリーニング（FACILITY store）
│   └── TroubleSupport.md            # 騒音・設備トラブル対応（FACILITY store）
│
...
- [x] 施設案内用 Vector Store を作成し、`vector_db_files/FacilityGuide.md`・`vector_db_files/FrequentlyAskedQuestions.md`・`vector_db_files/GuestServices.md`・`vector_db_files/TroubleSupport.md` をアップロードする
- [x] 交通・周辺・観光案内用 Vector Store を作成し、`vector_db_files/AccessMap.md` をアップロードする
- [x] 各 Store ID を SAM テンプレートに環境変数として追加する
  - `OPENAI_VECTOR_STORE_ID_FACILITY` — `vs_6852985443fc81919289d99a85d77cff`
  - `OPENAI_VECTOR_STORE_ID_TRANSPORT` — `vs_69db17f015188191a5dd8bc82564c948`

### Step 2: Intent Classifier の新規実装

- [x] `lambda_functions/ResponseApi/codes/intent_classifier.ts` を新規作成する
  - モデル: `gpt-5.4-nano`（非ストリーミング）reasoning effort: medium
  - 分類カテゴリ: `facility` / `transport_tourism` / `combined` / `emergency` / `conversation` / `unknown`
  - `previous_response_id` を常に渡してコンテキストを維持する
  - json schema で返り値を `{ intent: string }` に固定
  - `combined`: 施設と交通の両方にまたがる質問（例：荷物を預けてUSJへ行きたい）
  - `unknown` は完全に意味不明なメッセージ（文字化け等）にのみ使用

### Step 3: Intent → Tools マッピング定義

- [x] Intent ごとの tools 設定を `stream_response.ts` 内のヘルパー関数として実装する
  - `resolveVectorStoreIds(intent)`: Vector Store ID 配列を返す
  - `shouldUseWebSearch(intent)`: web_search 有効化フラグを返す
...
- [x] `GenerateStreamResponseParams` に `intent` フィールドを追加する
- [x] Intent に応じて `tools` と `vector_store_ids` を動的に切り替えるロジックを実装する

### Step 5: chat_handler.ts のフロー実装

- [x] Step 1（Intent 分類）→ Step 2（ストリーミング応答）の 2 ステップフローを実装する
- [x] `intent_classifier.ts` の呼び出しを `chat_handler.ts` に組み込む
- [x] エラー時のフォールバック intent を `unknown` とする
- [x] `operator_transfer_classifier.ts` を新規作成し、`classifyIntent` と `Promise.all` で並列実行する
  - モデル: `gpt-5.4-nano`、reasoning effort: medium、フォールバック: `true`（安全側）
  - `needsOperatorCheck=true` の場合のみ転送スキーマ・`OPERATOR_CALL_INSTRUCTION` を含める
- [x] `intent === "emergency"` の場合は classifier 結果に関わらず `needsOperatorCheck` を強制 `true` にオーバーライドする

### Step 6: System Prompt の整理

- [x] `getToolUsageInstruction(intent)` 関数を実装し、intent 別に指示ブロックを切り替える
  - `facility` → `FILE_SEARCH_NOTE`（file_search 優先の注意書き）
  - `transport_tourism` → `WEB_SEARCH_NOTE`（Google Map URL 禁止、テキストで案内）
  - `combined` → 両方
  - `emergency` → `EMERGENCY_INSTRUCTION`（原因推測・DIY修理禁止、即転送案内、電話番号積極開示）
  - `conversation` / `unknown` → `""` （0 トークン）
- [x] `JSON_OUTPUT_INSTRUCTION` を簡略化（operator_transfer_classifier に判定責任を移管、主 AI はユーザーの転送合意時のみ `needs_human_operator=true` にセット）
- [x] `needsOperatorCheck` による `OPERATOR_CALL_INSTRUCTION` の条件付き挿入を実装する
- [x] `getSystemPrompt()` のシグネチャに `intent` と `needsOperatorCheck` を追加する

### Step 7: SAM テンプレートの更新

- [x] `template-responseapi-function.yaml` に以下の環境変数を追加する
  - `OPENAI_VECTOR_STORE_ID_FACILITY`
  - `OPENAI_VECTOR_STORE_ID_TRANSPORT`
- [x] `template-twilio-functions.yaml` に `OPENAI_VECTOR_STORE_ID_FACILITY` を追加する
- [x] `responseapi-deploy.yml`・`twilio-deploy.yml` の GitHub Actions を更新する
